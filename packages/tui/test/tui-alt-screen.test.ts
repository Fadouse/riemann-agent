...
describe("TuiAltScreen", () => {
	...
	it("keeps search navigation and highlights correct across theme, document, query, and size changes", async () => {
		...
		terminal.sendInput("needle");
		await terminal.waitForRender();
		assert.ok(terminal.getViewport().some((line) => line.includes("needle") && line.includes("1/2")));
		assert.ok(
			terminal.events.some((event) => event.type === "write" && event.data.includes("\x1b[42mneedle\x1b[49m")),
		);
		assert.ok(
		...
		tui.requestRender();
		await terminal.waitForRender();
		assert.ok(terminal.getViewport().some((line) => line.includes("needle") && line.includes("2/3")));
		...
		await terminal.waitForRender();
		assert.ok(terminal.getViewport().some((line) => line.includes("gamma") && line.includes("1/1")));
	...
	it("clears stale iTerm2 image placements when they leave the viewport", async () => {
		...
		try {
			const terminal = new RecordingTerminal(20, 3);
			const tui = new TuiAltScreen(terminal);
			const imageLine = "\x1b]1337;File=inline=1;size=4;width=2;height=auto:AAAA\x07";
			tui.addChild({
