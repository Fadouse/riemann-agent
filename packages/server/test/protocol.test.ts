...
function connect(host = new TestServerHost()): ProtocolTestClient {
	server = new Server(host, { listeners: [], serverId: "00000000-0000-4000-8000-000000000001" });
...
test("routes image prompt payloads without dropping detail", async () => {
	const host = new TestServerHost();
	await host.seed("session-1");
	const client = connect(host);
	await client.hello();
	await expect(client.attach("00000000-0000-4000-8000-000000000001", "session-1")).resolves.toMatchObject({
		ok: true,
	});
	...
	await expect(
		client.requestSessionService("00000000-0000-4000-8000-000000000001", "session-1", call),
	).resolves.toMatchObject({ ok: true });
	expect(host.latestHarness("session-1").serviceCalls).toEqual([call]);
});
