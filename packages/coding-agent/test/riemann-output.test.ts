...
test("retains omitted rows and only advances text that fits the final model message", async () => {
	...
	const prepared = await views.prepare({ $riemann: "output_view", value: rows, sources: [] }, 1, 1024);
	expect(prepared.type).toBe("output_ref");
	if (prepared.type !== "output_ref") throw new Error("Expected deferred view");
	const first = await renderModelText(views, ["header".repeat(200), prepared], 1024);
	...
	for (let steps = 0; cursor && steps < 100; steps++) {
		const next = await renderModelText(restored, [await restored.more(cursor)], 1024);
		expect(Buffer.byteLength(next.text)).toBeLessThanOrEqual(1024);
...
test("continues retained stdout without losing capture status and transfers authorized views", async () => {
	...
	const full = `α😀${"BODY".repeat(600)}END`;
	...
	let output = "";
	for (let steps = 0; cursor && steps < 100; steps++) {
		const next = await renderModelText(parentViews, [await parentViews.more(cursor)], 1024);
	...
	expect(output.match(/BODY/g)).toHaveLength(600);
...
test("does not reinterpret user text or send binary content to the model", async () => {
	...
	if (prepared.type !== "text") throw new Error("Expected small output");
	expect(prepared.text).toContain(literal);
	expect(prepared.text).toContain(`binary resource ${ref(data)}`);
	expect(prepared.text).not.toContain("/wBB");
