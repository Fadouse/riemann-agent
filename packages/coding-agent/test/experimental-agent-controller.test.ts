...
describe("AgentController service", () => {
	test("provides the presentation-safe AgentLane command surface", async () => {
		...
		const steerImage = { type: "image", data: "c3RlZXI=", mimeType: "image/webp" } as const;
		...
		try {
				...
				host.services.invoke(
					{
						...
						member: "prompt",
						args: [{ message: "hello", images: [promptImage] }],
					},
					BACKGROUND_CONTEXT,
				),
				...
				host.services.invoke(
					{
						serviceId: AgentController.id,
						member: "steer",
						args: [{ message: "later", images: [steerImage] }],
					},
					BACKGROUND_CONTEXT,
				),
			...
			expect(prompt).toHaveBeenCalledWith("hello", [promptImage], BACKGROUND_CONTEXT);
			expect(requestAbort).toHaveBeenCalledWith("operation-1", BACKGROUND_CONTEXT);
			expect(steer).toHaveBeenCalledWith("later", [steerImage], BACKGROUND_CONTEXT);
