...
Running Subagents appear in the Fleet below the editor with exactly one leading icon. An active row uses a hollow `○` until selected, when it becomes `●`; settled rows instead retain their lifecycle outcome icon (`✓`, `✗`, or `■`). Running rows show the Agent name, its latest response preview or current task, elapsed time, and tokens; settled rows collapse to the Agent name and outcome icon, linger for four seconds, then disappear. With an empty editor, press `Left` or `Down` to focus the Fleet, use `Up`/`Down` to select an Agent, and press `Enter` to open its conversation viewer.
