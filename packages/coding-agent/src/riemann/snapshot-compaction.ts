...
import type { Api, ImageContent, Message, TextContent } from "@earendil-works/pi-ai";
import { stripAnsi } from "../utils/ansi.ts";
import { renderSnapshotPng, snapshotSupportedChars } from "./snapshot-native.ts";

const INTENT_FIELD = "i";

type AnthropicKind = "opus" | "sonnet" | "fable" | "mythos";

function parseAnthropicModel(modelId: string): { kind: AnthropicKind; version: string } | null {
	...
	if (kindFirst) return { kind: kindFirst[1] as AnthropicKind, version: kindFirst[2].replaceAll("-", ".") };
	if (versionFirst) return { kind: versionFirst[2] as AnthropicKind, version: versionFirst[1].replaceAll("-", ".") };
	return null;
}

function semverGte(left: string, right: string): boolean {
...
function createPathTreeNode(): PathTreeNode {
	return { files: [], fileNames: new Set(), subdirs: [], dirIndex: new Map() };
}

function formatGroupedPaths(paths: readonly string[], annotate?: (path: string) => string): string {
	const root = createPathTreeNode();
	for (const rawPath of paths) {
		const normalized = rawPath.replace(/\\/g, "/");
		if (/^[a-z][a-z0-9+.-]*:\/\//i.test(normalized)) {
			if (!root.fileNames.has(normalized)) {
				root.fileNames.add(normalized);
				root.files.push({ name: normalized, key: rawPath });
			}
			continue;
		}
		...
		if (name && !node.fileNames.has(name)) {
			node.fileNames.add(name);
			node.files.push({ name, key: rawPath });
		}
	}
	const lines: string[] = [];
	const walk = (node: PathTreeNode, depth: number): void => {
		for (const file of node.files) lines.push(annotate ? `${file.name}${annotate(file.key)}` : file.name);
		for (const subdir of node.subdirs) {
			let child = subdir.node;
			const parts = [subdir.name];
			while (child.files.length === 0 && child.subdirs.length === 1) {
				parts.push(child.subdirs[0].name);
				child = child.subdirs[0].node;
			}
...
function renderFileOperations(files: string): string {
	return `<files>\n${files}\n</files>`;
}

function renderSnapshotSummary(values: {
	frameCount: number;
	docColumns: boolean;
	cols: string | number;
	rows: number;
	...
	if (values.frameCount > 0) {
		...
		lines.push(
			values.docColumns
				...
				: `  - Frame: one grid ${values.cols} characters wide, up to ${values.rows} rows tall; read left→right, top→bottom. No word wrap; words may break across rows.`,
		);
		if (values.sentenceInk) lines.push("  - Ink: six colors, one per sentence.");
		if (values.stopwordDimmed) lines.push("  - Function words: dim gray; content words: full ink.");
		if (values.lineRepeated)
			lines.push("  - Each line printed twice (white, then pale-yellow band); copies identical.");
	}
	if (values.includedPreviousSummary) {
		lines.push("- HISTORY opens with a condensed digest of still-older context predating archived turns.");
	}
	if (values.truncatedChars > 0) {
		lines.push(`- About ${values.truncatedChars} characters of older middle history dropped to fit archive budget.`);
	}
	lines.push(
	...
	if (values.files) lines.push("FILES", "===================", values.files, "");
	lines.push("HISTORY", "===================", "");
	return lines.join("\n");
}
...
function formatFileOperations(readFiles: string[], modifiedFiles: string[], readSet?: ReadonlySet<string>): string {
	...
	return files.length > 0 ? renderFileOperations(files) : "";
}
...
export function serializeConversation(messages: Message[], options?: SerializeOptions): string {
	...
	for (const msg of messages) {
		...
		if ((msg as typeof msg & { useless?: boolean }).useless === true && msg.isError !== true) {
		...
		if (msg.role === "user") {
			...
			for (const block of msg.content) {
				if (block.type === "text") {
					...
					// Render the stable intent argument as a one-line `//comment` and
					// drop it from the serialized arguments below.
					const rawIntent = typeof args[INTENT_FIELD] === "string" ? (args[INTENT_FIELD] as string) : "";
...
function renderableUnicodeChars(chars: readonly string[], font: Shape["font"] | undefined): ReadonlySet<string> {
	...
	const supported = new Set(snapshotSupportedChars(primaryFont, text));
	if (primaryFont !== "silver") {
		for (const ch of snapshotSupportedChars("silver", text)) supported.add(ch);
	}
	return supported;
}

function normalizedInputChars(text: string): string[] {
	const stripped = text.includes("\u001b") ? stripAnsi(text) : text;
...
export async function render(text: string, shape: Shape, size: number = shape.frameSize): Promise<RenderedFrame> {
	...
	const data = await renderSnapshotPng(text, nativeRenderOptions(shape, size));
