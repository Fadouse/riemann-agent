...
- Execution automatically yields while still running. Call `ipython_wait` with the returned `cell_id` to get new output or completion. Do not repeat the producer. Collect the cell before starting another.
- The optional first-line `# @exec: {"timeout_ms": 300000}` sets the only execution deadline, covering the cell and nested operations. Waiting does not reset it. Use `terminate=true` on `ipython_wait` to cancel; cancellation cannot undo completed side effects.
- Each tool return contains at most 50 KB (51200 UTF-8 bytes) of text, including status and references. Complete output is retained. Use `await output.more(ref="...")` for a `[more ...]` reference; each continuation follows the same bound.
