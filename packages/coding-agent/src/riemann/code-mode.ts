...
export function parsePythonExec(source: string): PythonExecOptions {
	...
	for (const [key, item] of Object.entries(value)) {
		if (key === "timeout_ms" || key === "yield_time_ms") {
			const [min, max] = key === "timeout_ms" ? [1, 86400000] : [0, 60000];
...
export class PythonCells {
	...
	get pending(): Array<{ cell_id: string; status: "running" | "awaiting_collection"; next_tool: string }> {
		return [...this.cells.values()].map((cell) => ({
			cell_id: cell.id,
			status: cell.result ? ("awaiting_collection" as const) : ("running" as const),
	...
	start(options: PythonExecOptions, execute: (cell: PythonCell) => Promise<KernelExecuteResult>): string {
		...
		const cell: ActiveCell = {
			...
			controller,
			details: { status: "running", startedAt: Date.now(), taskKey: randomUUID() },
	...
	async poll<T>(
		...
		try {
			...
			while (!cell.result) {
				if (!terminate && cell.yieldVersion > cell.collectedYield) break;
			...
			const yieldVersion = cell.yieldVersion;
			...
			// Kernel completion can remove its live preview before done reaches this
			// owner. An absent preview must never rewind already delivered offsets.
			cell.stdout = Math.max(cell.stdout, current.stdout.length);
			cell.stderr = Math.max(cell.stderr, current.stderr.length);
			cell.displays = Math.max(cell.displays, current.displays.length);
			cell.modelContent = Math.max(cell.modelContent, current.modelContent.length);
			cell.collectedYield = yieldVersion;
