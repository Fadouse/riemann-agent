	...
	private finish(
		...
		if (activity.kind === "mcp") {
			...
			activity = {
				...
				stdoutTruncated: value?.stdout_truncated === true,
				stderrTruncated: value?.stderr_truncated === true,
				stdoutCaptureTruncated: value?.stdout_capture_truncated === true,
				stderrCaptureTruncated: value?.stderr_capture_truncated === true,
				stdoutArtifactHandle: stringValue(objectValue(value?.stdout_artifact)?.handle),
				stderrArtifactHandle: stringValue(objectValue(value?.stderr_artifact)?.handle),
				exitCode,
				timedOut,
