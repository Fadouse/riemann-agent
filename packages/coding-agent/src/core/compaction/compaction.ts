...
function buildSummarizationContext(promptText: string, sourceContext?: Context, images: ImageContent[] = []): Context {
...
export async function generateSummaryWithUsage(
	...
	let promptText = "";
	let summaryImages: ImageContent[] = [];
	if (!sourceContext) {
		const llmMessages = convertToLlm(currentMessages);
		const conversationText = serializeConversation(llmMessages);
		promptText = `<conversation>\n${conversationText}\n</conversation>\n\n`;
		if (model.input.includes("image")) {
			summaryImages = collectConversationImages(llmMessages);
		}
	}
	...
	const response = await completeSummarization(
		model,
		buildSummarizationContext(promptText, sourceContext, summaryImages),
		completionOptions,
		streamFn,
		retry,
		callbacks,
