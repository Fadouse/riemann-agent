...
export async function processFileArguments(fileArgs: string[], options?: ProcessFileOptions): Promise<ProcessedFiles> {
	...
	for (const fileArg of fileArgs) {
		...
		// Stat once for existence and the empty-file fast path.
		...
		if (mimeType) {
			...
			if (!processed.ok) {
				textParts.push(`<file name="${absolutePath}">${processed.message}</file>\n`);
				continue;
			}
			...
			if (processed.hints.length > 0) {
				textParts.push(`<file name="${absolutePath}">${processed.hints.join("\n")}</file>\n`);
			} else {
