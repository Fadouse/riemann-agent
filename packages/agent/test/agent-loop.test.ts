...
import { AgentToolExecutionError, setDefaultStreamFn } from "../src/index.ts";
...
describe("agentLoop with AgentMessage", () => {
	...
	it.each(["ok", "structured error", "ordinary error"])(
		"should handle tool calls and results: %s",
		async (outcome) => {
			...
			const tool: AgentTool<typeof toolSchema, { value: string }> = {
				...
				async execute(_toolCallId, params) {
					...
					if (outcome === "structured error") throw new AgentToolExecutionError(result);
					if (outcome === "ordinary error") throw new Error("tool failed");
					return result;
				},
			};
			...
			if (toolEnd?.type === "tool_execution_end") {
				expect(toolEnd.isError).toBe(outcome !== "ok");
				expect(toolEnd.result.content).toEqual([
					{ type: "text", text: outcome === "ordinary error" ? "tool failed" : "echoed: hello" },
				]);
				expect(toolEnd.result.details).toEqual(outcome === "ordinary error" ? {} : { value: "hello" });
			}
			expect(observedToolUsage).toEqual(outcome === "ordinary error" ? undefined : toolUsage);
			const messages = await stream.result();
