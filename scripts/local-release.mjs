...
const packages = [
	...
	{ directory: "packages/coding-agent", name: "riemann-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "riemann-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `riemann-${platform}.zip` : `riemann-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createRiemannShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "riemann.cmd"))) {
			writeFileSync(join(installDirectory, "riemann.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\riemann.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "riemann.ps1"), '& "$PSScriptRoot/node_modules/.bin/riemann.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "riemann.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\riemann.exe" %*\r\n');
		writeFileSync(join(installDirectory, "riemann.ps1"), '& "$PSScriptRoot/node_modules/.bin/riemann.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "riemann"), join(installDirectory, "riemann"));
}
...
if (rootPackageJson.name !== "riemann-agent-monorepo") {
...
if (!options.skipInstall) {
	...
	createRiemannShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createRiemannShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `riemann-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "riemann.exe" : "riemann")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "riemann.cmd" : "riemann")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "riemann.cmd" : "riemann")} --help`);
	}
}
